...
function addsubitem(parentlevel)
 ...
 function editCat(thisID)
	{
	window.location = '?module=category_edit&tblname=<?=$tblname?>&catID='+thisID;
	}
...
/////////////////////showchangepic    
function changepic(olevel)
	{
	if (confirm("<?php echo $strChangePic; ?> ?")==true)
		{
		if (confirm("<?php echo $strConfirm.' '.$strAgain; ?> !!!")==true)
			{
			document.tree.action.value="changepic";
			document.tree.level.value=olevel;
			document.tree.submit();
			}
		}
 ...
 /////////////////////showchangeurl
function changeurl(olevel)
	{
	if (confirm("<?php echo $strChangeUrl; ?> ?")==true)
		{
		if (confirm("<?php echo $strConfirm.' '.$strAgain; ?> !!!")==true)
			{
			document.tree.action.value="changeurl";
			document.tree.level.value=olevel;
			document.tree.submit();
			}
		}
 ...
 /////////////////////showchangechoise 
function changechoise(olevel)
	{
	if (confirm("<?php echo $strSaveChange; ?> ?")==true)
		{
		if (confirm("<?php echo $strConfirm.' '.$strAgain; ?> !!!")==true)
			{
			document.tree.action.value="changechoise";
			document.tree.level.value=olevel;
			document.tree.submit();
			}
		}
...
/////////////////////showchangemetadata 
function changemeta(olevel)
	{
	if (confirm("<?php echo $strSaveChange; ?> ?")==true)
		{
		if (confirm("<?php echo $strConfirm.' '.$strAgain; ?> !!!")==true)
			{
			document.tree.action.value="changemeta";
			document.tree.level.value=olevel;
			document.tree.submit();
			}
		}
...
if (isset($_REQUEST['tblname']) and !in_array($_REQUEST['tblname'],$system_tbl))
    ...
    if($tblname == 'news')
        $showChoise = true;
    $titleChoise = $strHideInMenu;
    $fieldChoise = 'hidemenu';
    //show change meta data
    $showchangemeta = false;
    if(in_array($tblname,array("articles","products")))$showchangemeta = true;
    ////////////////////////////////////////////////////////////////////////////////////////////////////////
	if (isset($_POST['action']))
		...
		if ($dotree and mysql_num_rows($dotree)>=0)
			...
			switch ($action)
				...
				if (isset($_POST['level'],$_POST['subname']))
					...
					if ($docheck)
                        ...
                        /////////////////////showchangepic save
                        if(isset($_POST['new_pic']) && $showchangepic)$query .= ',image';
                        /////////////////////showchangeurl
                        if(isset($_POST['new_url']) && $showchangeurl)$query .= ',url';
                        /////////////////////////show choise
                        if($showChoise)$query .= ','.$fieldChoise;
						//show change meta 
                        if($showchangemeta)$query .= ',title,keyword,description';
                        $query .= ')';
						$query.=' values("'.$_POST['subname'].'","'.$newlevel.'","'.get_langID().'"';
                        /////////////////////showchangepic save
                        if(isset($_POST['new_pic']) && $showchangepic)$query .=',"'.$_POST['new_pic'].'"';
                        /////////////////////showchangeurl
                        if(isset($_POST['new_url']) && $showchangeurl)$query .=',"'.$_POST['new_url'].'"';
                        ///////////////////show choise
                        if($_POST[$fieldChoise] == 1 && $showChoise)$query .=',"1"';
                        else if($showChoise)$query .=',"0"';
                        //show change meta data
                        if($showchangemeta)$query .= ',"'.addslashes($_POST['new_title']).'","'.addslashes($_POST['new_keyword']).'","'.addslashes($_POST['new_description']).'"';
						$query .= ')';
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
                ...
                case 'changepic':
				//Kiem tra
				if (isset($_POST['level']) && isset($_POST['pic_'.$_POST['level']]))
					{
					   $this_id=$_POST['level'];
                       $this_image=$_POST['pic_'.$this_id];
                       $this_sql='update '.$tblname.'_cat set image="'.$this_image.'" where id="'.$this_id.'"';
                       //$msg.=$this_sql;
                       if (mysql_query($this_sql,$link))
									{
									$msg.=$strSuccess;
									}
								else
                ...
                case 'changeurl':
				//Kiem tra
				if (isset($_POST['level']) && isset($_POST['url_'.$_POST['level']]))
					{
					   $this_id=$_POST['level'];
                       $this_url=$_POST['url_'.$this_id];
                       $this_sql='update '.$tblname.'_cat set url="'.$this_url.'" where id="'.$this_id.'"';
                       //$msg.=$this_sql;
                       if (mysql_query($this_sql,$link))
									{
									$msg.=$strSuccess;
				...
				break;
				//show change metadata
                case 'changemeta':
				//Kiem tra
				if (isset($_POST['level']) 
                        && isset($_POST['title_'.$_POST['level']],
                                    $_POST['keyword_'.$_POST['level']],
                                    $_POST['description_'.$_POST['level']]))
					{
					   $this_id=$_POST['level'];
                       $this_title=$_POST['title_'.$this_id];
                       $this_keyword=$_POST['keyword_'.$this_id];
                       $this_description=$_POST['description_'.$this_id];
                       $this_sql='update '.$tblname.'_cat set ';
                       $this_sql .= 'title ="'.stripslashes($this_url).'" ';
                       $this_sql .= ' where id="'.$this_id.'"';
                       $dosql = do_sql($tblname."_cat",array("id"=>$this_id,
                                                             "title"=>$this_title,
                                                             "keyword"=>$this_keyword,
                                                             "description"=>$this_description),'update');
                       //$msg.=$this_sql;
                       if ($dosql)
									{
									$msg.=$strChangeMeta." ".$strSuccess;
									}
								else
									{
									$msg.='<font color="red">'.$strChangeMeta." ".$strErro.'</font>';
									}
					}
				break;
                ///////////////////////////////showchangechoise
				...
				if (isset($_POST['level']))
					{
					   $this_id=$_POST['level'];
                       //print_r($_POST);
                       if(isset($_POST['choise_'.$this_id]))$this_choise=$_POST['choise_'.$this_id];
                       else $this_choise= '0';
					   $this_sql='update '.$tblname.'_cat set '.$fieldChoise.'="'.$this_choise.'" where id="'.$this_id.'"';
                       //$msg.=$this_sql;
                       if($this_choise == 0) $this_choise = ' ' . $strGroup . ' \' '. stripslashes(get_by_id($tblname."_cat",$this_id,"name")) .' \' \' => \'OFF\' ';
                       else $this_choise = ' ' . $strGroup . ' \' '. stripslashes(get_by_id($tblname."_cat",$this_id,"name")) .' \' => \' ON \' ';
                       $this_choise = $titleChoise." : ".$this_choise;
                       $msg .= $this_choise."<br/>";
                       if (mysql_query($this_sql,$link))
									{
									$msg.=$strSuccess;
		...
		if (isset($msg))
			...
			<td style="width:auto; height: 30px;" colspan="3" align="center" valign="middle">
				...
				<h3 class="ui-widget-header"> <?php echo $strProgress; ?></h3>
				<div class="ui-widget-body" style="text-align: left;">
				<a class="back_button" href="javascript:void(0);" onclick="window.history.go(-1);"><?=$strBack?></a>
				<br clear="both" /><br /><br />
                <?php
				echo $msg;
				//echo $redirect;
		...
		<table style="width: 100%;" cellspacing="0" cellpadding="0">
          <tr>
			...
			<table style="width: 100%;" cellspacing="0" cellpadding="0">
			  <tr>
				<td align="center" valign="top" style="padding: 5px; background: none repeat scroll 0% 0% rgb(255, 255, 255); border: 1px solid #C5DBEC;">
				...
				?>
                <span class="treetext" style="cursor: pointer;" onClick="window.location='?module=quickedit&tblname=<?php echo $tblname; ?>&type=quickedit'"><img src="images/page_white_text.gif" alt="" />&nbsp;<?php echo $strViewAll." ".$strRecord; ?></span>&nbsp;&nbsp;
				...
				<?php
				$tree ='select * from '.$tblname.'_cat';
				if($tblname!='gallery')$tree.=' where lang="'.get_langID().'"';
                else $tree.=' where 1 ';
				...
				if ($dotree and mysql_num_rows($dotree)>0)
					...
					while ($result=mysql_fetch_array($dotree))
                        ...
                        ///////////////////showchoise 
                        if($showChoise)$choise_array[$i] = $result[$fieldChoise];
                        /////////////////////showchangepic
                        if($showchangepic)$image_array[$i]=$result['image'];
                        ///////////////////////////////showchangeurl
                        if($showchangeurl)$url_array[$i]=$result['url'];
						//show change meta data
                        if($showchangemeta)
                        {
                            $title_array[$i] = $result['title'];
                            $keyword_array[$i] = $result['keyword'];
                            $description_array[$i] = $result['description'];
					...
					while (isset($id_array[$i]))
						...
						<input class="invisible editable" type="text" value="<?php echo $name_array[$i]; ?>" id="<?php echo $id_array[$i]; ?>"  name="item<?php echo $id_array[$i]; ?>" onFocus="javascript: this.style.border='solid 1px #336699';" style='display:none; height:22px;' />
						...
						<tr>
						<TD align="center" onclick="window.location='?module=quickedit&SID=<?php echo session_id(); ?>&tblname=<?php echo $tblname; ?>&catname=<?php echo $name_array[$i]; ?>&catID=<?php echo $id_array[$i]; ?>'" class="button_off" onMouseOver="this.className='button_over';" onMouseOut="this.className='button_off';">&raquo; <?php echo $strView; ?></TD>
						...
						if ($_SESSION['usergroup']<2)
							{
							 if($showdetailedit){
							     ?>
                            <TD align="center" onClick="editCat('<?php echo $id_array[$i];?>');" class="button_off" onMouseOver="this.className='button_over';" onMouseOut="this.className='button_off';">
							&raquo; <?php echo $strEditDetail; ?></TD>
                                 <?php
							 }
							 ?>
							...
							if ($_SESSION['usergroup']<2)
								...
								<?php if($showchangepic){ 
                                /////////////////////showchangepic
                                ?>
                            
                                <td colspan="2" align="center">
                                    	
                                    <input style="width:180px;" class="longinput" type="text" value="<?php echo $image_array[$i]; ?>" name="pic_<?php echo $id_array[$i]; ?>" onFocus="blur()" onClick="show_popup('<?php echo $_SESSION['language']; ?>','images','<?php echo $_GET['tblname']; ?>',this.form.name,this.name)" /> 
                                </td><td>
                                    <input class="pointer" type="button" value="<?php echo $strChangePic; ?>" onclick="changepic(<?php echo $id_array[$i]; ?>)" />
                                    
                                </td>
                            
                            <?php } if($showchangeurl){  ?>
                            
                                <td colspan="2" align="center">
                                    <input style="width:180px;" class="longinput" type="text" value="<?php echo $url_array[$i]; ?>" name="url_<?php echo $id_array[$i]; ?>" /> 
                                </td><td>
                                    <input class="pointer" type="button" value="<?php echo $strChangeUrl; ?>" onclick="changeurl(<?php echo $id_array[$i]; ?>)" />
                                </td> <?php }if($showChoise){  ?>
                            
                                <td align="center">
                                    <label for="choise_<?=$id_array[$i]?>"><?=$titleChoise?></label><input type="checkbox" value="1" name="choise_<?=$id_array[$i]?>" <?php if($choise_array[$i] == 1)echo ' checked="checked" '; ?> /> 
                                </td><td>
                                    <input class="pointer" type="button" value="<?php echo $strSave; ?>" onclick="changechoise(<?php echo $id_array[$i]; ?>)" />
                                </td> <?php } ?>
                            </tr>
							<tr>
                                <td colspan="10">
                                    <?php if($showchangemeta){  ?>
                                    <h5 style="font-size: 11px; padding: 3px; margin: 0pt;"><?=$strMeta?></h5>
                                    <div class="attribute ctitle">
                                    <span>Title</span>
                                        <input style="width:180px;" class="longinput" type="text" value="<?php echo $title_array[$i]; ?>" name="title_<?php echo $id_array[$i]; ?>" /> 
                                    </div>
                                    <div class="attribute ckeyword">
                                    <span>Keyword</span>
                                    <textarea name="keyword_<?php echo $id_array[$i]; ?>"><?=$keyword_array[$i]?></textarea>
                                    </div>
                                    <div class="attribute cdescription">
                                    <span>Description</span>
                                    <textarea name="description_<?php echo $id_array[$i]; ?>"><?=$description_array[$i]?></textarea>
                                    </div>
                                    <input class="button" type="button" value="<?php echo $strSave; ?>" onclick="changemeta(<?=$id_array[$i]?>)" />
                                    <?php } ?>
                                </td>
                            </tr>
							<?php
...
bobexample.init()
			  ...
			  <tr><td style="border-left: solid 0px #336699; border-right: solid 0px #336699; border-bottom: solid 0px #336699;">
				...
				<input class="" type="submit" style="display: none;" name="Save" value=" <?php echo $strSaveChange; ?> " />
			...
			<form name="addsub" method="post" action="" style="display: none;" onsubmit="return add_cate();">
    			...
    			<div class="adminform ui-widget-content">
                    <h3 class="ui-widget-header">&raquo; <?php echo $strAdd.' '.$strSubCategory; ?></h3>
                    <div class="ui-widget-body">
                      <span class="label"><?php echo $strName; ?> :</span> 
                      <input type="text" name="subname" class="longinput" />
                      <div class="clear"></div>
                      <?php if($showchangepic){ /////////////////////showchangepic ?>
                      ...
                      <div class="clear"></div>
    				  <?php } ?>
                      <?php if($showchangeurl){ /////////////////////showchangeurl ?>
					  ...
					  <?php if($showchangemeta){ ?>
                        ...
                        <input class="longinput" type="text" value="<?php echo $title_array[$i]; ?>" name="new_title" /> 
                      <div class="clear"></div>
                        <span class="label">Keyword</span>
                        <textarea class="longinput" name="new_keyword"><?=$keyword_array[$i]?></textarea>
                      <div class="clear"></div>
                        <span class="label">Description</span>
                        <textarea class="longinput" name="new_description"><?=$description_array[$i]?></textarea>
                      <div class="clear"></div>
                      <?php } ?>
                      <p class="bigtitle" style="margin-top: 12px; margin-bottom: 12px;">
    					...
    					&nbsp;&nbsp;&nbsp;
    					<input type="button"  class="button" style="cursor:pointer;" name="Cancel" value=" <?php echo $strBack; ?> " onClick="addsubitem();" />
                      </p>
                    </div>
                </div>
			</form>
